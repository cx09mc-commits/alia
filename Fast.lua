--[[
    الملف الكامل والمكمل (بدون نقص حرف)
    متوافق مع Ubuntu 22.04
--]]
...
myip=io.popen([[echo `curl -s 'https://api.ipify.org' `]]):read('*a')
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(Fast.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(Fast) then
return false
end
end
...
if msg.sender.user_id == tonumber(Fast) then
return false
end
...
Redis:del(Fast..msg.sender.user_id.."bottoken")
Redis:del(Fast..msg.sender.user_id.."botuser")
Redis:del(Fast..msg.sender.user_id.."make:bot")
...
-----تشغيل البوتات ---
if text and Redis:get(Fast..msg.sender.user_id.."run:bot") then
Redis:del(Fast..msg.sender.user_id.."run:bot")
...
Redis:set(Fast..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"※ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(Fast..msg.sender.user_id.."stop:bot") then
Redis:del(Fast..msg.sender.user_id.."stop:bot")
Redis:del(Fast.."screen:on")
Redis:del(Fast.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(Fast..msg.sender.user_id.."stop:bot",true)
...
if text and Redis:get(Fast..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(Fast..msg.sender.user_id.."bottoken")
local botuser = Redis:get(Fast..msg.sender.user_id.."botuser")
...
if text and Redis:get(Fast..msg.sender.user_id.."make:bot") == "token" then
...
Redis:set(Fast..msg.sender.user_id.."botuser",botuser)
Redis:set(Fast..msg.sender.user_id.."bottoken",text)
Redis:set(Fast..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(Fast..msg.sender.user_id.."make:bot","token")
...
-------delete 
if text and Redis:get(Fast..msg.sender.user_id.."make:bot") == "del" then
...
Redis:set(Fast..msg.sender.user_id.."make:bot","del")
...
----end--3dd
if text and Redis:get(Fast..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '※ الغاء الامر' then   
Redis:del(Fast..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n※ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
...
Redis:set(Fast..msg.sender.user_id.."setchannel",true)
...
--brodcast
if Redis:get(Fast..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '※ الغاء الامر' then   
Redis:del(Fast..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n※ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(Fast.."total") 
...
if text == "※ اذاعه" then
Redis:set(Fast..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"※ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(Fast..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '※ الغاء الامر' then   
Redis:del(Fast..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(Fast..msg.sender.user_id.."brodcast:fwd",true)
...
if msg.sender.user_id ~= tonumber(Fast) then
if Redis:sismember(Fast..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(Fast.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text == "/start" then
if not Redis:sismember(Fast.."total",msg.sender.user_id) then
Redis:sadd(Fast.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
send(msg.chat_id,msg.id,"※ اهلا بك في صانع بوتات سورس فاست \n※ مطور البوت @MahmoudM2","md",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(Fast..msg.sender.user_id.."make:bot") then
if text == "※ الغاء" then
Redis:del(Fast..msg.sender.user_id.."make:bot")
...
local url = https.request("http://api.telegram.org/bot"..text.."/getme")
local json_data = JSON.decode(url)
if json_data.ok == true then
local botuser = json_data.result.username
if Redis:sismember(Fast.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n※ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(Fast..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(Fast.."userbots",botuser)
Redis:del(Fast..msg.sender.user_id.."make:bot")
...
if text == "※ صنع بوت" then
if Redis:get(Fast..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"※ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(Fast..msg.sender.user_id.."make:bot",true)
...
if Redis:get(Fast..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(Fast..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(Fast.."userbots",botuser)
Redis:del(Fast..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n※ تم حذف البوت بنجاح","md",true)  
else
